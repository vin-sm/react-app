    ...
    return (
      <div className='recommended-card'>
        ...
        <div className='recommended-card-details'>
          ...
          <div>
            <button className={(card.seatLeft <=0) ? 'disableButton' : 'enableButton'} onClick={onClick} disabled={(card.seatLeft <=0 || card.isEnrolled) ? true : false}>
                {card.isEnrolled == true ? 'Enrolled' : 'Enroll'}
            </button>
            </div>
          </div>
        </div>
